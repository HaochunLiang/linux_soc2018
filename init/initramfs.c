...
static char * __init unpack_to_rootfs(char *buf, unsigned long len)
{
	...
	pr_info("header_buf:%x\n",header_buf);
	symlink_buf = kmalloc(PATH_MAX + N_ALIGN(PATH_MAX) + 1, GFP_KERNEL);
	pr_info("symlink_buf:%s:\n",*symlink_buf);
	pr_info("symlink_buf:%x:\n",symlink_buf);
	...
	while (!message && len) {
		...
		pr_info("decompress\n");
		pr_debug("Detected %s compressed data\n", compress_name);///pr_debug->pr_info()，这个地方存在问题
		if (decompress) {
...
static int __init populate_rootfs(void)
{
	...
	pr_info("__initramfs_size:%x",__initramfs_size);
	pr_info("__initramfs_start(addr):%x",(int)__initramfs_start);
